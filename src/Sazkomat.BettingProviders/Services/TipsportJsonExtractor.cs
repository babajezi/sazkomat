...
/// <summary>
/// Extracts and parses JSON data from Tipsport.cz REST API
/// Uses FlareSolverr (primary) or Playwright (fallback) to bypass Cloudflare protection
/// </summary>
public class TipsportJsonExtractor
{
    private readonly ILogger<TipsportJsonExtractor> _logger;
    ...
    public TipsportJsonExtractor(
        ILogger<TipsportJsonExtractor> logger,
    ...
    /// <summary>
    /// Extracts competition data from Tipsport for a given date range
    /// </summary>
        ...
        DateTimeOffset? dateTo = null)
    {
        try
        {
            ...
            _logger.LogInformation("Fetching Tipsport data from {From} to {To}", from, to);
            ...
            if (!jsonResult.IsSuccess)
            {
                return Result<List<TipsportCompetition>>.Failure(jsonResult.Error);
            }

            var parseResult = ParseJson(jsonResult.Value);
            if (!parseResult.IsSuccess)
            {
                return Result<List<TipsportCompetition>>.Failure(parseResult.Error);
            }

            var competitions = ExtractCompetitionsFromTree(parseResult.Value);

            _logger.LogInformation("Successfully extracted {Count} competitions from Tipsport", competitions.Count);

            return Result<List<TipsportCompetition>>.Success(competitions);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error extracting Tipsport data");
    ...
    /// <summary>
    /// Fetches JSON from Tipsport API using FlareSolverr (Cloudflare bypass)
    /// </summary>
    private async Task<Result<string>> FetchJsonViaFlareSolverrAsync()
    {
        ...
        try
        {
            _logger.LogInformation("Fetching Tipsport via FlareSolverr...");

            // Step 1: Get session cookies by visiting the football page
            var cookieResult = await _flareSolverrClient.GetSessionCookiesAsync($"{BaseUrl}{FootballPageUrl}");
            if (!cookieResult.IsSuccess)
            {
                return Result<string>.Failure($"Failed to get session cookies: {cookieResult.Error}");
            }

            var cookies = cookieResult.Value;
            _logger.LogDebug("Got {Count} session cookies", cookies.Count);

            // Step 2: Fetch API with cookies
            var apiUrl = $"{BaseUrl}{ApiEndpoint}?fromResults=false&withLive=true";
            var apiResult = await _flareSolverrClient.GetAsync(apiUrl, cookies);
            ...
            if (string.IsNullOrWhiteSpace(json) || !json.TrimStart().StartsWith("{"))
            {
                _logger.LogWarning("Invalid response from FlareSolverr: {Preview}",
                    json?.Substring(0, Math.Min(200, json?.Length ?? 0)));
                return Result<string>.Failure("Invalid JSON response from Tipsport");
            }

            _logger.LogInformation("FlareSolverr: Got {Length} bytes from Tipsport API", json.Length);
    ...
    /// <summary>
    /// Fetches JSON from Tipsport API using Playwright to bypass Cloudflare (fallback)
    /// </summary>
    private async Task<Result<string>> FetchJsonViaPlaywrightAsync(DateTimeOffset from, DateTimeOffset to)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Navigating to Tipsport football betting page...");
            ...
            for (int attempt = 0; attempt < 6; attempt++)
            {
                ...
                if (pageContent.Contains("tipsport") || pageContent.Contains("Fotbal") ||
                    pageContent.Contains("kurzy") || pageContent.Contains("odds"))
                {
                    _logger.LogInformation("Page loaded successfully (detected Tipsport content)");
                    pageReady = true;
                    break;
                }

                _logger.LogDebug("Page content check {Attempt}: waiting for Tipsport content...", attempt + 1);
            }
            ...
            if (string.IsNullOrEmpty(capturedJson))
            {
                return Result<string>.Failure("Failed to capture JSON response from Tipsport API");
            }
        ...
        {
            _logger.LogError(ex, "Playwright request failed for Tipsport");
    ...
    /// <summary>
    /// Parses JSON response into TipsportResponse
    /// </summary>
    private Result<TipsportResponse> ParseJson(string json)
    {
        try
        {
            ...
            _logger.LogDebug("Parsed Tipsport response with {Count} children",
        ...
        {
            _logger.LogError(ex, "Failed to parse Tipsport JSON");
