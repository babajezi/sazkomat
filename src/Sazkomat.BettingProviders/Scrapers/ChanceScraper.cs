...
/// <summary>
/// Scraper for Chance.cz betting provider.
/// Extracts league data from REST API via FlareSolverr/Playwright (to bypass Cloudflare).
///
/// Note: Chance and Tipsport are both owned by SAZKA Group and use same API structure.
/// Chance uses Czech names without country hierarchy (e.g., "1. anglická liga").
/// Country mapping is resolved via ICountryMappingService (database-driven) with fallback to hardcoded dictionary.
/// </summary>
public class ChanceScraper : IBettingProviderScraper
{
    private readonly ChanceJsonExtractor _jsonExtractor;
    private readonly ICountryMappingService _countryMappingService;
    private readonly ILogger<ChanceScraper> _logger;
    private const string BaseUrl = "https://www.chance.cz";

    public string ProviderCode => "chance";

    public ChanceScraper(
        ChanceJsonExtractor jsonExtractor,
        ICountryMappingService countryMappingService,
        ILogger<ChanceScraper> logger)
    {
    ...
    public async Task<Result<List<LeagueAvailability>>> GetAvailableLeaguesAsync(string sportCode)
    {
        ...
        try
        {
            _logger.LogInformation("Fetching available leagues from Chance for sport: {SportCode}", sportCode);

            // Map sport code to SuperSportId (same as Tipsport)
            var superSportId = MapSportCodeToSuperSportId(sportCode);
            if (superSportId == null)
            {
                return Result<List<LeagueAvailability>>.Failure(
                    $"Sport code '{sportCode}' not supported by Chance scraper");
            }

            // Extract competitions from Chance
            ...
            _logger.LogInformation("Successfully extracted {Count} leagues from Chance for {SportCode}",
        ...
        {
            _logger.LogError(ex, "Error scraping Chance for sport {SportCode}", sportCode);
            return Result<List<LeagueAvailability>>.Failure($"Failed to scrape Chance: {ex.Message}");
        }
    }

    /// <summary>
    /// Transforms Chance competitions to LeagueAvailability entities.
        ...
        string sportCode)
    {
        ...
        foreach (var comp in competitions)
        {
            ...
            leagues.Add(new LeagueAvailability
            {
                ...
                ProviderLeagueId = comp.Id.ToString(CultureInfo.InvariantCulture),
    ...
    {
        // Try database mappings first - use "chance" provider code
        ...
        // Fall back to Tipsport mappings (same language/format)
        (code, name) = await _countryMappingService.ResolveCountryAsync("tipsport", title);
        if (code != null)
        {
            return (code, name);
        }
    ...
    /// <summary>
    /// Maps internal sport code to SuperSportId (same as Tipsport)
    /// </summary>
    ...
    /// Uses same mappings as Tipsport since both use Czech naming conventions.
    /// </summary>
    private static (string? Code, string? Name) DeriveCountryFromTitleFallback(string title)
    {
        // Common patterns in Chance competition names (same as Tipsport)
        ...
        var specialCases = new Dictionary<string, (string Code, string Name)>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "Chance Liga", ("czech-republic", "Česko") },  // Chance-specific naming
