...
export default function CountryMappingsPage() {
  ...
  const [editingMapping, setEditingMapping] = useState<CountryNameMapping | undefined>(undefined);
  ...
  const { data: mappings, isLoading, error } = useQuery({
    ...
    queryFn: () => countryMappingApi.getMappings(),
  });

  // Delete mutation
  const deleteMutation = useMutation({
    mutationFn: (id: string) => countryMappingApi.deleteMapping(id),
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: string) => countryMappingApi.toggleMappingActive(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["country-mappings"] });
    },
  });
  ...
  const filteredMappings = (mappings || []).filter((m) => {
    ...
    if (
      searchQuery &&
      !m.providerCountryName.toLowerCase().includes(searchQuery.toLowerCase()) &&
      !m.betExplorerCode.toLowerCase().includes(searchQuery.toLowerCase())
    ) {
      return false;
    }
    return true;
  ...
  const handleEdit = (mapping: CountryNameMapping) => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Mapování Názvů Zemí</h1>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <Input
                placeholder="Název země nebo kód"
                value={searchQuery}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="px-4 py-3 text-left text-sm font-medium">BetExplorer Code</th>
                ...
                <tbody>
                  {paginatedMappings.map((mapping) => (
                    <tr key={mapping.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3 text-sm">{mapping.providerCountryName}</td>
                      <td className="px-4 py-3 text-sm font-mono text-xs uppercase">{mapping.betExplorerCode}</td>
